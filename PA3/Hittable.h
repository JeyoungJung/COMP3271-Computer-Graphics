...
public:
    ...
    Triangle(const Point &a,
             ...
             const Vec &n_b,
             const Vec &n_c,
             bool phong_interpolation = true) : a_(a), b_(b), c_(c), n_a_(n_a), n_b_(n_b), n_c_(n_c), phong_interpolation_(phong_interpolation)
    {
        // Calculate the area and the normal
        Vec ba = a_ - b_;
        Vec bc = c_ - b_;
        area = 0.5 * glm::length(glm::cross(ba, bc));
    }
